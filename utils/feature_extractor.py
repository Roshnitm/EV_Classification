...
feature_extractor = MobileNetV2(
    weights="imagenet", #based on the features we are using the pretrained model to extract features from the images, so we set weights to "imagenet" to use the pre-trained weights on the ImageNet dataset.
    include_top=False, #this is for avoiding the border in the image
    pooling="avg" 
)
